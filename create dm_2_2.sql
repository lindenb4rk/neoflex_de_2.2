CREATE
OR REPLACE PROCEDURE dm_create_loan () LANGUAGE PLPGSQL AS $$
DECLARE 
lines bigint;
BEGIN
truncate dm.loan_holiday_info;

CALL logs_insert('Truncate  dm.loan_holiday_info');
...
insert into dm.loan_holiday_info SELECT deal_rk
      ,effective_from_date
      ,effective_to_date
      ,agreement_rk
	  ,account_rk
...
CALL logs_insert('ADD in dm.loan_holiday_info:'||lines||' lines');
